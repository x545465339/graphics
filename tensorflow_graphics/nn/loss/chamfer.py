...
def evaluate(point_set_a, point_set_b, name=None):
  ...
  Args:
    point_set_a: A tensor of shape `[A1, ..., An, N, D]`, where the last axis
      represents the position of the point in a D dimensional space.
    point_set_b: A tensor of shape `[A1, ..., An, M, D]`, where the last axis
      represents the position of the point in a D dimensional space.
    name: A name for this op. Defaults to "chamfer_evaluate".

  Returns:
    A tensor of shape `[A1, ..., An]`, where the last axis represents the
    chamfer distance between the two point sets.

  Raises:
    ValueError: if the shape of `point_set_a`, `point_set_b` is not supported.
  """
  with tf.compat.v1.name_scope(name, "chamfer_evaluate",
    ...
    d = point_set_a.shape.as_list()[-1]
    shape.check_static(
        tensor=point_set_b, tensor_name="point_set_b", has_dim_equals=(-1, d))
